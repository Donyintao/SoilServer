...
// 故障级别列表
$(function () {
  $.get("/faults/level_seleted/", function (result) {
...
// 故障类型列表
$(function () {
  $.get("/faults/class_seleted/", function (result) {
    var result = $.parseJSON(result);
    var type = $('#get_type').val();
    for (var i = 0; i < result.length; i++) {
      var id = result[i].id;
      var name = result[i].name;
      $("#type option[index='0']").remove();
      if (name == type) {
        $("#type").append("<option value=" + id + " selected=selected" + ">" + name + "</option>")
      } else {
...
// 影响项目列表
$(function () {
  $.get("/faults/project_seleted/", function (result) {
    var result = $.parseJSON(result);
...
// 故障状态列表
$(function () {
  $.get("/faults/status_seleted/", function (result) {
    var result = $.parseJSON(result);
...
// 主导改进列表
$(function () {
  $.get("/faults/improve_seleted/", function (result) {
    var result = $.parseJSON(result);
        ...
        function (result, status) {
          if (status == 'success') {
            ...
            if (result.status == 'true') {
              bootbox.alert("<h4 class='text-center'>数据添加成功!</h4>", function(){ self.location='/faults/list/'; });
            }
          }
        });
    } else {
      ...
      $.post('/faults/update/', {
        ...
        function (result, status) {
          if (status == 'success') {
            var result = $.parseJSON(result);
            if (result.status == 'true') {
              bootbox.alert("<h4 class='text-center'>数据更新成功!</h4>", function(){ self.location='/faults/list/'; });
            }
          }
        });
    } else {
  ...
  $('.del_fault').click(function () {
    ...
    bootbox.confirm({
      ...
      callback: function (result) {
        if (result) {
          $.post("/faults/delete/", {id: id}, function (result,status) {
            if (status == 'success') {
              var result = $.parseJSON(result);
              if (result.status == 'true') {
