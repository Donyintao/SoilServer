  ...
  $('.del_dns').click(function () {
    ...
    bootbox.confirm({
      ...
      callback: function (result) {
        if (result) {
          $.post("/bind/delete/", {id: id}, function (result,status) {
            if (status == 'success') {
...
$(function () {
  ...
  $('.add_dnsValid').click(function () {
    ...
    if($('#add_dnsFrom').data("bootstrapValidator").isValid()){
      ...
      var type = $('#type').val();
      var data = $('#data').val();
      $.post('/bind/add/', {host: host, zone: zone, type: type, data: data}, function (result, status) {
        if (status == 'success') {
          var result = $.parseJSON(result);
          if (result.status == 'true') {
            bootbox.alert("<h4 class='text-center'>数据添加成功!</h4>", function(){ self.location='/bind/list/'});
          }
        }
      });
    } else {
...
$(function () {
  ...
  $('.up_dnsValid').click(function () {
    ...
    if($('#up_dnsForm').data("bootstrapValidator").isValid()){
      ...
      var type = $('#type').val();
      var data = $('#data').val();
      $.post('/bind/update/', {id: id, zone: zone, host: host, type: type, data: data},  function (result, status) {
        if (status == 'success') {
          var result = $.parseJSON(result);
          if (result.status == 'true') {
            bootbox.alert("<h4 class='text-center'>数据更新成功!</h4>", function(){ self.location='/bind/list/'});
          }
        }
      });
    } else {
